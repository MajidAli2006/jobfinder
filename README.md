...
| Flag | What it does |
|---|---|
| `--days 7` | Only adverts posted in the last 7 days. Left out, the window is read from `--query` — "listed in the last 5 days" searches five — and falls back to 30 |
...
| `--no-cache` | Judge every advert afresh rather than reusing a stored verdict. Slower, and costs API calls |
...
.venv/bin/python -m pytest tests/ -q      # 790 tests, fully offline
