...
class FreshnessTests(unittest.TestCase):
    ...
    def setUp(self):
        self.cutoff, *_ = freshness_window(config.FRESHNESS_DAYS)

    def test_today_accepted(self):
        self.assertTrue(filters.check_freshness(posting(days_ago=0), self.cutoff).passed)

    def test_inside_window_accepted(self):
        inside = max(0, config.FRESHNESS_DAYS - 2)
        self.assertTrue(filters.check_freshness(posting(days_ago=inside), self.cutoff).passed)

    def test_outside_window_rejected(self):
        verdict = filters.check_freshness(posting(days_ago=config.FRESHNESS_DAYS + 1), self.cutoff)
        self.assertFalse(verdict.passed)
    ...
    def test_missing_date_is_kept_and_flagged(self):
        job = posting()
        job.posted_at = None
        verdict = filters.check_freshness(job, self.cutoff)
        self.assertTrue(verdict.passed)
