...
class RedirectsAreVettedTests(unittest.TestCase):
    """Where an address redirects to is as untrusted as the address itself.

    The model proposes a URL and these checks clear it. They say nothing about
    where that URL redirects to, and requests follows redirects by default — so
    a public host answering 302 to 169.254.169.254 was followed and its body
    read, which is exactly the hole this module exists to close.
    """

    def test_a_redirect_target_is_judged_by_the_same_rules(self):
        from job_agent.discovery import url_rejection
        for blocked in ("https://169.254.169.254/x", "https://127.0.0.1/x",
                        "https://localhost/x", "http://example.com/x",
                        "https://user:pass@example.com/x", "https://example.com:22/x"):
            with self.subTest(url=blocked):
                self.assertTrue(url_rejection(blocked), f"{blocked} should be refused")

    def test_a_real_board_address_is_still_allowed(self):
        from job_agent.discovery import url_rejection
        self.assertEqual(url_rejection("https://www.jobberman.com/jobs?q=x"), "")

    def test_a_redirect_into_a_private_address_is_not_followed(self):
        from unittest.mock import Mock, patch
        from job_agent import discovery

        hop = Mock(is_redirect=True, headers={"location": "https://127.0.0.1/internal"})
        with patch.object(discovery, "url_rejection", side_effect=discovery.url_rejection):
            with patch("job_agent.utils.http_get", return_value=hop) as get:
                self.assertIsNone(discovery.safe_get("https://www.jobberman.com/jobs?q=x"))
        self.assertEqual(get.call_count, 1, "the private hop is never requested")

    def test_a_redirect_to_a_public_address_is_followed(self):
        from unittest.mock import Mock, patch
        from job_agent import discovery

        hop = Mock(is_redirect=True, headers={"location": "https://www.jobberman.com/jobs/2"})
        final = Mock(is_redirect=False)
        with patch("job_agent.utils.http_get", side_effect=[hop, final]) as get:
            self.assertIs(discovery.safe_get("https://www.jobberman.com/jobs?q=x"), final)
    ...
    def test_a_redirect_loop_gives_up(self):
        ...
        hop = Mock(is_redirect=True, headers={"location": "https://www.jobberman.com/loop"})
        with patch("job_agent.utils.http_get", return_value=hop) as get:
            self.assertIsNone(discovery.safe_get("https://www.jobberman.com/loop"))
        self.assertLessEqual(get.call_count, discovery.MAX_REDIRECTS + 1)
