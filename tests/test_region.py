...
class LinkedInGeoSelectionTests(unittest.TestCase):
    ...
    def test_a_country_without_a_geo_id_is_searched_by_name(self):
        # A country LinkedIn has no ready id for (most of the world) must be
        # searched by name, not replaced with the default spread of others.
        for country in ("Pakistan", "Nigeria", "Brazil", "Liechtenstein"):
            names = self._geos(target_regions=(country,), home_country=country)
            self.assertIn(country, names)
            self.assertIn("Worldwide", names)
            self.assertLess(len(names), len(self.LinkedIn.GEOS))
            self.assertNotIn("United Kingdom", names)

    def test_a_named_country_is_searched_by_id_not_name(self):
        from dataclasses import replace
        profile.set_active(replace(self.base, key="compiled:test",
                                   target_regions=("Pakistan",),
                                   home_country="Pakistan"))
        geos = {name: geo_id for name, geo_id in self.LinkedIn().geos()}
        # Pakistan has no id, so it is searched by name; Worldwide keeps its id.
        self.assertEqual(geos["Pakistan"], "")
        self.assertEqual(self.LinkedIn._geo_param("Pakistan", ""), {"location": "Pakistan"})
        self.assertEqual(self.LinkedIn._geo_param("United Kingdom", "101165590"),
                         {"geoId": "101165590"})
