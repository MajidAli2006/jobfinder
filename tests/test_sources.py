...
from job_agent import config, profile
...
class BroadRegionRoutingTests(unittest.TestCase):
    """Naming somewhere broad must not buy less coverage than naming nowhere.

    Only a country can be checked against a board's markets. A continent or
    "worldwide" matched none of them, so a Europe-wide search dropped Adzuna,
    Reed and a board actually called EU Remote Jobs — and stating "Worldwide"
    returned fewer sources than leaving the region blank.
    """

    def tearDown(self):
        profile.reset()

    def _serves(self, *regions):
        """Which market-scoped boards answer this search.

        Asked of the boards themselves rather than of `build_sources`, which
        also drops anything lacking an API key — that would make the result
        depend on whether the machine running the tests has credentials.
        """
        from job_agent.sources import ALL_SOURCE_CLASSES
        search_in(*regions) if regions else search_in("")
        return {cls.name for cls in ALL_SOURCE_CLASSES
                if cls.markets and cls().serves_active_search()}

    BROAD = ("Worldwide", "Anywhere", "Europe", "Asia", "Africa", "North America")

    def test_a_broad_region_keeps_everything_an_unstated_one_would(self):
        unstated = self._serves("")
        self.assertTrue(unstated, "some boards are market-scoped")
        for region in self.BROAD:
            with self.subTest(region=region):
                self.assertEqual(self._serves(region), unstated)

    def test_a_country_still_routes_to_the_boards_that_serve_it(self):
        uk = self._serves("United Kingdom")
        self.assertIn("reed", uk)
        self.assertIn("adzuna", uk)

    def test_a_country_still_excludes_boards_that_do_not_serve_it(self):
        nigeria = self._serves("Nigeria")
        self.assertNotIn("reed", nigeria)
        self.assertNotIn("adzuna", nigeria)

    def test_one_named_country_is_enough_to_keep_a_board(self):
        self.assertIn("reed", self._serves("United Kingdom", "Nigeria"))


class JoobleLocationRetryTests(unittest.TestCase):
    """Jooble's country filter answers for some markets and not others.

    Asking it for "Pakistan" or "Karachi" returned nothing while "Lahore"
    returned plenty, so the whole source went quiet for countries it covers.
    An empty located search is retried without the location.
    """

    def tearDown(self):
        profile.reset()

    def _jooble(self, responses):
        from unittest.mock import patch
        from job_agent.sources.market_boards import Jooble
        search_in("Pakistan")
        source = Jooble()
        source.enabled = True
        calls = []

        def fake_post(query, location=None):
            calls.append(location)
            return responses[len(calls) - 1] if len(calls) <= len(responses) else {}

        with patch.object(source, "_post", side_effect=fake_post):
            data = source._search("electrician")
        return data, calls

    def test_a_located_search_that_answers_is_left_alone(self):
        data, calls = self._jooble([{"jobs": [{"id": 1}]}])
        self.assertEqual(len(data["jobs"]), 1)
        self.assertEqual(calls, [None], "no retry when the location works")

    def test_an_empty_located_search_is_retried_without_the_location(self):
        data, calls = self._jooble([{"jobs": []}, {"jobs": [{"id": 2}, {"id": 3}]}])
        self.assertEqual(len(data["jobs"]), 2)
        self.assertEqual(calls, [None, ""], "the retry drops the location")

    def test_a_search_with_no_location_is_not_retried(self):
        from unittest.mock import patch
        from job_agent.sources.market_boards import Jooble
        search_in("")
        source = Jooble()
        source.enabled = True
        with patch.object(source, "_post", return_value={"jobs": []}) as post:
            source._search("electrician")
        self.assertEqual(post.call_count, 1, "nothing to drop, so nothing to retry")


class ServerSideFreshnessTests(unittest.TestCase):
    """Boards that filter by posting age must be asked for the run's window.

    All three used to ask for the 30-day default however few days the run was
    reading, so a 7-day search paid to download three weeks it then discarded.
    """

    def tearDown(self):
        profile.reset()

    def _reading(self, days: int) -> None:
        profile.set_active(replace(profile.active(), key="compiled:test",
                                   freshness_days=days))

    def test_indeed_asks_for_the_window_the_run_is_reading(self):
        self._reading(7)
        self.assertEqual(Indeed()._params("flutter", 50)["fromage"], 7)

    def test_ziprecruiter_asks_for_the_window_the_run_is_reading(self):
        self._reading(7)
        self.assertEqual(ZipRecruiter()._params("flutter", 100)["days_ago"], 7)

    def test_adzuna_asks_for_the_window_the_run_is_reading(self):
        self._reading(5)
        with mock.patch.object(config, "HTTP_DELAY", 0), \
             mock.patch("job_agent.sources.market_boards.get_json",
                        return_value={}) as fetched:
            Adzuna()._results("gb", "flutter")
        self.assertEqual(fetched.call_args.kwargs["params"]["max_days_old"], 5)

    def test_the_default_window_is_what_they_ask_for_when_nothing_narrowed_it(self):
        self.assertEqual(Indeed()._params("flutter", 50)["fromage"],
