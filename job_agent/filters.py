...
def check_freshness(raw: RawJob, cutoff: datetime, days: int) -> Verdict:
    """Posted within the last `days` calendar days, Europe/London.

    `days` is the window `cutoff` was built from, and is required so that the
    reasons written here cannot describe a window the run never used.
    """
    ...
    if posted < cutoff:
        ...
        return Verdict(
            False,
            f"Posted {age} days ago (outside the {days}-day window)",
            "stale")
...
def check_pay_floor(job, floor_usd: float = config.SALARY_FLOOR_USD,
    ...
    value, basis = annual_usd(job, profile.active().home_country)
