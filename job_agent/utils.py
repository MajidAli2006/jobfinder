...
def http_get(url: str, *, params: dict | None = None, headers: dict | None = None,
    ...
    `allow_redirects=False` returns the redirect itself rather than following
    it, so a caller that must vet each hop can do so.
    """
    ...
    for attempt in range(attempts + 1):
        try:
            ...
            if resp.status_code == 200 or (not allow_redirects and resp.is_redirect):
                return resp
...
#: Addresses that reach nobody, or reach a machine.
UNCONTACTABLE_MAILBOXES = ("example.com", "sentry", "noreply", "no-reply",
                           "donotreply", "do-not-reply", "mailer-daemon")

#: Addresses that reach a person, but never the one doing the hiring. Writing
#: to a company's abuse desk about a vacancy wastes the application.
ROLE_MAILBOXES = frozenset({
    "postmaster", "abuse", "privacy", "security", "webmaster", "hostmaster",
    "dmarc", "dpo", "legal", "unsubscribe", "billing", "accounts",
})
...
def first_email(text: str) -> str:
    ...
    for email in found:
        ...
        if any(low.endswith(ext) for ext in (".png", ".jpg", ".jpeg", ".gif", ".svg", ".webp")):
            continue
        if any(bad in low for bad in UNCONTACTABLE_MAILBOXES):
            continue
        if low.split("@")[0] in ROLE_MAILBOXES:
            continue
...
#: Schemes a report may turn into something clickable. Everything else —
#: `javascript:`, `data:`, `file:` — is shown as text instead.
LINKABLE_SCHEMES = ("http://", "https://", "mailto:")


def safe_url(url: str) -> str:
    """The URL if a report may link to it, otherwise "".

    Adverts come from third parties and carry their own apply links, so a
    posting could offer `javascript:...` and have the report render it as a
    button the reader is invited to click.
    """
    text = (url or "").strip()
    return text if text.lower().startswith(LINKABLE_SCHEMES) else ""
...
#: Longest first: "R$" is a Brazilian real and contains a dollar sign, so a
#: shorter match would read a São Paulo salary as US dollars.
_CURRENCY_SYMBOLS = {"R$": "BRL", "zł": "PLN", "kr": "SEK", "£": "GBP",
                     "$": "USD", "€": "EUR", "₹": "INR", "¥": "JPY"}
#: Every currency with a rate, longest code first so no code masks another.
#: Listing only the Western handful left a Lagos or Karachi advert with no
#: currency at all, and therefore no comparable figure.
_CURRENCY_CODES = tuple(sorted(config.CURRENCY_TO_USD, key=len, reverse=True))
...
#: Below this, a bare number in an advert is noise rather than a year's pay —
#: a headcount, a postcode, a phone extension. Read in the advert's own units,
#: not converted: currencies with small units simply quote larger numbers.
PLAUSIBLE_ANNUAL_FLOOR = 12_000

#: Above this in US dollars, a figure is not one person's salary. Only the
#: ceiling is converted, because it is the one end that means the same thing
#: everywhere.
PLAUSIBLE_ANNUAL_CEILING_USD = 1_000_000


def _within(values: list[float], low: float, high: float) -> tuple[float, float] | None:
    """The lowest and highest figure that could be pay at this scale, or None."""
    kept = [v for v in values if low <= v <= high]
    return (min(kept), max(kept)) if kept else None


def _plausible_annual(currency: str) -> tuple[float, float]:
    """The believable annual range for an advert quoting this currency.

    Judging the top end in dollars discarded whole markets — 8,000,000 yen and
    1,200,000 rupees are ordinary salaries that both sit above a million, so
    those adverts were reported as publishing no pay at all. Judging the bottom
    end in dollars was just as wrong in the other direction: it assumed nobody
    earns less than twelve thousand a year, which is most of the world.
    """
    rate = config.CURRENCY_TO_USD.get((currency or "USD").upper()) or 1.0
    return PLAUSIBLE_ANNUAL_FLOOR, PLAUSIBLE_ANNUAL_CEILING_USD / rate
...
def parse_compensation(text: str) -> dict[str, Any]:
    ...
    is_monthly = bool(re.search(
        r"(per month|/\s*month|a month|monthly|\bpm\b|per mo\b|/mo\b|p\.m\.)", low))
    ...
    result["currency"] = currency
    floor, ceiling = _plausible_annual(currency)

    if is_hourly:
        band = _within(values, 8, 500)
        if band:
            result["day_rate_min"] = round(band[0] * 8)
            result["day_rate_max"] = round(band[1] * 8)
            return result

    if is_day_rate:
        band = _within(values, 80, 4_000)
        if band:
            result["day_rate_min"], result["day_rate_max"] = band
            return result

    # Gulf, South Asian and much of African advertising quotes a month's pay.
    # Read as a year's it understates by twelve, which drops genuinely
    # well-paid work below a salary floor.
    if is_monthly:
        band = _within(values, floor / 12, ceiling / 12)
        if band:
            result["salary_min"], result["salary_max"] = band[0] * 12, band[1] * 12
            return result

    band = _within(values, floor, ceiling)
    if band:
        result["salary_min"], result["salary_max"] = band
    return result
