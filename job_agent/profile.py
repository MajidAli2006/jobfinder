...
class SearchProfile:
    ...
    #: Cities the request scoped on-site (and hybrid) work to. When set, office
    #: work outside these is not what was asked for; remote work is unaffected.
    #: Lowercased, matched the same way as home_city_terms. Empty leaves on-site
    #: scoping at country level.
    onsite_cities: tuple[str, ...] = ()
    ...
    #: How far back to read postings, in whole calendar days.
...
#: Number words a request uses in place of digits, so "the last seven days"
#: reads the same as "the last 7 days".
_WINDOW_NUMBER_WORDS = {
...
#: Years are deliberately absent: nobody wants a year-old advert, and "the
#: past 7 years" is how people describe their experience, not a window.
_WINDOW_UNIT_DAYS = {"hour": 1 / 24, "day": 1.0, "week": 7.0,
                     "fortnight": 14.0, "month": 30.0}

#: A number of days only means a window when the request says it is one. A
#: bare number is usually a shift pattern — "5 days a week" is not an ask for
#: postings from the last five days.
_WINDOW_CUE = (r"(?:last|past|previous|recent|within|inside|since|"
               r"newer than|no older than|not older than)")

_WINDOW_RE = re.compile(
    rf"\b{_WINDOW_CUE}\s+(?:the\s+)?"
    rf"(\d{{1,4}}|{'|'.join(sorted(_WINDOW_NUMBER_WORDS, key=len, reverse=True))})?"
    rf"\s*({'|'.join(sorted(_WINDOW_UNIT_DAYS, key=len, reverse=True))})s?\b"
)

#: Windows named without a number. "today" and "yesterday" need the posting
#: verb with them — "I can start today" says nothing about advert age.
_WINDOW_PHRASES = (
    (re.compile(r"\b(?:posted|listed|added|published|advertised)\s+today\b"), 1),
    (re.compile(r"\b(?:posted|listed|added|published|advertised)\s+"
                r"(?:since\s+)?yesterday\b"), 2),
    (re.compile(r"\bthis\s+week\b"), 7),
    (re.compile(r"\bthis\s+month\b"), 30),
    (re.compile(r"\bfortnight\b"), 14),
)


def _clamp_window(days: int) -> int:
    return max(1, min(config.MAX_FRESHNESS_DAYS, int(days)))


def freshness_from_request(request: str) -> int | None:
    """The posting-age window a request asks for, in whole days.

    None when the request never says how recent it wants the adverts, which
    leaves the standing window alone. Anything shorter than a day rounds up
    to one: "the last 48 hours" is two days, "the last 12 hours" is one.
    """
    ...
    if match:
        ...
        if counted is None:
            number = 1
        elif counted.isdigit():
            number = int(counted)
        else:
            number = _WINDOW_NUMBER_WORDS[counted]
        return _clamp_window(math.ceil(number * _WINDOW_UNIT_DAYS[match.group(2)]))

    for pattern, days in _WINDOW_PHRASES:
        if pattern.search(text):
            return days
    return None


def freshness_window_days(stated: int = 0, request: str = "") -> int:
    """How far back one run should read postings, in whole days.

    A number the caller passed wins. Failing that the request speaks for
    itself — "listed in the last 7 days" is an instruction, not decoration —
    and failing that the standing search carries the default.

    `stated` of 0 — or any number below it — means the caller did not say,
    which is not the same as asking for the shortest window there is.
    """
    if stated > 0:
        return _clamp_window(stated)
    named = freshness_from_request(request)
    if named is not None:
        return named
    return _clamp_window(active().freshness_days)
...
def _builtin_profile() -> SearchProfile:
    ...
    return SearchProfile(
        ...
        freshness_days=_clamp_int(search.get("freshness_days"), 1,
                                  config.MAX_FRESHNESS_DAYS, config.FRESHNESS_DAYS),
    )
...
PROFILE_SCHEMA: dict[str, Any] = {
    ...
    "properties": {
        ...
        "onsite_cities": {"type": "array", "items": {"type": "string"}},
    ...
    "required": [
        ...
        "home_terms", "home_city_terms", "onsite_cities", "region_terms",
...
`onsite_cities` — the specific cities the request scoped **on-site** work to.
    This is a distinct restriction, NOT the same as `home_city_terms` (which
    just lists a country's cities for matching). Whenever the request names a
    city or cities the office work must be in, put them here, lowercased, with
    common spellings — even when the same request also allows remote work:
      "jobs in Lahore"                         -> ["lahore"]
      "on-site in Lahore only, or remote"      -> ["lahore"]   (remote stays open)
      "must be in Berlin or Munich"            -> ["berlin", "munich"]
      "operations roles in Karachi"            -> ["karachi"]
    Leave it EMPTY only when the request names no city — a country on its own
    ("jobs in Pakistan"), a remote-only search, or no place at all. A country is
    not a city. An empty list keeps on-site matching at country level; it never
    restricts remote work, which is judged on eligibility, not city.
...
#: How people actually name each arrangement.
_ARRANGEMENT_PHRASES = {
    "hybrid": ("hybrid", "part remote", "partly remote", "part-remote",
               "partially remote", "split week"),
    "onsite": ("onsite", "on site", "on-site", "office based", "office-based",
               "in office", "in-office", "in the office", "office",
               "in person", "in-person"),
    "remote": ("remote", "work from home", "work-from-home", "wfh",
               "telecommut", "fully distributed", "work from anywhere"),
}


#: "remote or hybrid" names two arrangements; "partially remote" names one.
_ALTERNATIVES = re.compile(r"\bor\b|\band\b|[/,|]")


def _arrangement(value) -> str:
    """One of any/remote/hybrid/onsite, defaulting to any.

    Read leniently: "remote only", "fully remote" and "wfh" all name the same
    arrangement. An unreadable value becomes "any", which returns every
    arrangement — so anything readable has to be read, or a stated preference
    is dropped in silence.

    Naming more than one is the same as naming none: the field holds a single
    value, and "any" is the honest reading of "remote or hybrid".
    """
    text = str(value or "").strip().lower()
    matched = {name for name, phrases in _ARRANGEMENT_PHRASES.items()
               if any(phrase in text for phrase in phrases)}
    if not matched:
        return "any"
    if len(matched) > 1 and _ALTERNATIVES.search(text):
        return "any"
    if "hybrid" in matched:
        # "partially remote" names the word remote but describes hybrid work.
        return "hybrid"
    if len(matched) == 1:
        return matched.pop()
    return "any"
...
def _from_payload(payload: dict, request: str, has_cv: bool) -> SearchProfile:
    ...
    return SearchProfile(
        ...
        onsite_cities=_terms(payload.get("onsite_cities"), 20),
        ...
        freshness_days=freshness_from_request(request) or config.FRESHNESS_DAYS,
    )
