...
def _row(job: Job) -> str:
    ...
    apply_link = (f'<a href="{_e(job.application_url)}" target="_blank" rel="noopener">Apply</a>'
                  if job.application_url else "—")
...
<li><strong>Last {config.FRESHNESS_DAYS} calendar days</strong> in {_e(local_timezone().key)}
