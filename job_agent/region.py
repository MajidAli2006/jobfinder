...
def iso_countries() -> dict[str, str]:
    """Two-letter ISO code to country name, both lowercased.

    Callers matching against advert text want the same casing the rest of the
    place vocabulary uses.
    """
    return {code.lower(): name.lower()
            for code, name in _ISO_TO_COUNTRY.items() if len(code) == 2}
