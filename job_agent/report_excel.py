...
def _write_summary_sheet(wb: Workbook, result: RunResult) -> None:
    ...
    metric("Freshness window", f"Last {config.FRESHNESS_DAYS} calendar days")
    ...
    metric(f"Jobs older than {config.FRESHNESS_DAYS} days rejected", stats.rejected_stale)
    ...
    for rule in _filter_rules(search):
...
def _filter_rules(search) -> tuple[str, ...]:
    ...
    return (
        ...
        f"Posted within the last {config.FRESHNESS_DAYS} calendar days.",
...
def build_workbook(result: RunResult, path: Path) -> Path:
    ...
    wb.properties.description = (
        f"{_summary_subtitle(search)} Posted in the last {config.FRESHNESS_DAYS} days."
    )
