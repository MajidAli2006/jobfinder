...
def preview_search(query: str, cv_text: str = "", cv_path: str = "",
    ...
    if compiled.onsite_cities:
        cities = ", ".join(c.title() for c in compiled.onsite_cities)
        where += f" (on-site scoped to {cities}; remote unrestricted)"
    questions = ""
    if result.questions:
        questions = "\n".join(
    ...
    return "\n".join([
        ...
        f"- **Region:** {where}",
        f"- **Posted within:** the last {compiled.freshness_days} days",
...
def find_jobs(query: str = "", cv_text: str = "", cv_path: str = "",
              region: str = "", days: int = 0, min_salary_usd: float = 0.0,
              tier: str = "normal", offline: bool = False) -> str:
    """Search every configured job board and produce a ranked Excel workbook.

    `days` is how far back to read postings. Leave it at 0 and the window is
    taken from the request itself — "listed in the last 7 days" searches
    seven — falling back to 30 days when the request does not say.
    """
...
def _find_jobs(query: str, cv_text: str, cv_path: str, region: str, days: int,
    ...
    window = profile.freshness_window_days(days, query)
    ...
    lines = [
        ...
        "",
        f"Searched {stats.sources_searched} sources over the last {window} days, "
