...
class LinkedIn(Source):
    ...
    def geos(self) -> tuple[tuple[str, str], ...]:
        ...
        A geo is (name, id). Countries LinkedIn has a ready id for are searched
        by id; a country with no id — anywhere outside the table below — is
        searched by name, so a request scoped to it queries that country rather
        than the default spread of unrelated ones. With no country named at all,
        the whole table is searched.
        """
        from .. import profile

        active = profile.active()
        countries = tuple(active.target_regions) or (
            (active.home_country,) if active.home_country else ())
        if not countries:
            return self.GEOS
        ...
        chosen = [g for g in self.GEOS if g[0] in keep]
        have = {name.lower() for name, _ in chosen}
        chosen += [(c, "") for c in countries if c.lower() not in have]
        return tuple(chosen)

    @staticmethod
    def _geo_param(name: str, geo_id: str) -> dict:
        """Search by LinkedIn's geo id where there is one, else by place name."""
        return {"geoId": geo_id} if geo_id else {"location": name}
    ...
    def _search_tasks(self) -> list[tuple[str, dict, dict | None, str]]:
        """Every (keywords, geo params, extra params, label) search this run makes.

        Primary keywords sweep every region; the weaker sweeps are capped to the
        first few regions unless this is a deep run.
        """
        tasks: list[tuple[str, dict, dict | None, str]] = []
        all_geos = self.geos()
        for keywords in self.primary_keywords():
            for name, geo_id in all_geos:
                tasks.append((keywords, self._geo_param(name, geo_id), None, ""))

        secondary_geos = all_geos if self.deep else all_geos[: self.SECONDARY_GEOS]
        for keywords in self.secondary_keywords():
            for name, geo_id in secondary_geos:
                tasks.append((keywords, self._geo_param(name, geo_id), None, ""))
        ...
        for code, label in self.JOB_TYPES:
            for keywords in (self.JOB_TYPE_KEYWORDS or self.queries(4)):
                for name, geo_id in job_type_geos:
                    tasks.append((keywords, self._geo_param(name, geo_id),
                                  {"f_JT": code}, label))
        return tasks

    def _sweep(self, seen: dict[str, RawJob], seen_lock: Lock, keywords: str,
               geo: dict, extra: dict | None = None, label: str = "") -> None:
