...
#: Set False to read nothing back. Writes continue, so a run told to ignore
#: what is stored still leaves its own results for the next one.
ENABLED = True
...
def get(key: str, max_age_days: float) -> dict[str, Any] | None:
    ...
    try:
        ...
        return None
    # Every caller reads this with .get(). A row holding a list or a bare
    # string — a half-written entry, or one left by an older format — would
    # otherwise end the run in an AttributeError far from here.
