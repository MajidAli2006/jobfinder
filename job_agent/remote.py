...
from .geography import (
    home_city_terms, home_explicitly_eligible, home_mentioned,
    home_strongly_eligible, residency_patterns, worldwide, foreign_country_terms,
    home_known, country_label, home_label, location_country,
    region_excluding_home_terms, region_terms,
)
...
def _positive_eligibility(r: RemoteReading) -> RemoteVerdict:
    ...
    # A concrete foreign country in the advert's own location field is where the
    # work physically is. "Global" or "worldwide" appearing in the body usually
    # describes the employer ("a global leader in ..."), not the job, and must
    # not turn an office role in another country into a work-from-anywhere one.
    # Only a location-agnostic location field itself overrides a named country.
    if r.worldwide and (not based_in or worldwide(r.location)):
    ...
    if based_in:
        where = country_label(based_in)
        # The location field names a foreign country while the body claims
        # worldwide reach: a genuine conflict the words alone cannot settle, so
        # keep it as a prospect for the second reader rather than dropping it.
        conflicting = r.worldwide
        return RemoteVerdict(
            False,
            f"Remote role based in {where} with no {home} or worldwide "
            f"eligibility stated",
            "ineligible",
            remote_status=f"Remote — {where}",
            eligibility=f"Not eligible — advertised for {where}",
            prospect_worthy=conflicting,
        )
    return RemoteVerdict(
        False,
...
def unreachable(raw: RawJob, arrangement: str) -> RemoteVerdict | None:
    """Work requiring attendance in a country the candidate does not live in.

    `assess_remote` reads location only on the remote path, because a remote
    advert is the only one whose location can be argued with. Office-based work
    has to be physically reachable, which is the same eligibility question
    asked of a different arrangement, so it is answered here beside it.
    """
    based_in = location_country(raw.location_raw)
    if not based_in:
        return None
    where, label = country_label(based_in), arrangement_label(arrangement)
    return RemoteVerdict(
        False,
        f"{label} role based in {where} — not reachable from {home_label()}",
        "ineligible",
        remote_status=f"{label} — {where}",
        eligibility=f"Not eligible — the work is based in {where}",
    )


def wrong_onsite_city(raw: RawJob, arrangement: str) -> RemoteVerdict | None:
    """On-site work the search scoped to named cities, but based elsewhere.

    Runs only when the request named the cities its office work must be in, and
    after `unreachable` has already removed other countries — so the question
    left is narrower: within reach, is the office in a city that was asked for?
    A different named city is rejected; a location that names no city we can
    check is kept as a prospect for a human to confirm rather than dropped.
    Remote roles never reach this: they are settled on the remote path.
    """
    cities = profile.active().onsite_cities
    if not cities:
        return None
    location = normalize(raw.location_raw)
    label = arrangement_label(arrangement)
    wanted = ", ".join(c.title() for c in cities)

    if location and any(word_present(city, location) for city in cities):
        return None

    elsewhere = next((c for c in home_city_terms()
                      if c not in cities and word_present(c, location)), "")
    if elsewhere:
        return RemoteVerdict(
            False,
            f"{label} in {elsewhere.title()}, but this search wants on-site "
            f"work in {wanted}",
            "ineligible",
            remote_status=f"{label} — {elsewhere.title()}",
            eligibility=f"Not eligible — on-site work wanted in {wanted}",
        )
    return RemoteVerdict(
        False,
        f"{label} role, but the advert does not confirm it is in {wanted}",
        "ineligible",
        remote_status=f"{label} — location unconfirmed",
        eligibility=f"Unconfirmed — confirm the office is in {wanted}",
        prospect_worthy=True,
    )
