...
def build_job(raw: RawJob, verdict: remote.RemoteVerdict,
    ...
    job.salary_usd_equivalent = money.annual_usd(job, profile.active().home_country)[0]
...
def _reconcile_arrangement(raw: RawJob, verdict: remote.RemoteVerdict,
    ...
    if not verdict.passed and verdict.category == "not_remote":
        if remote.arrangement_wanted(arrangement):
            return (
                remote.unreachable(raw, arrangement)
                or remote.wrong_onsite_city(raw, arrangement)
                or remote.RemoteVerdict(
                    True, "", "",
                    remote_status=remote.arrangement_label(arrangement),
...
def screen(raws: list[RawJob], stats: RunStats, cutoff, run_date, *,
    ...
    for raw in raws:
        ...
        fresh = filters.check_freshness(raw, cutoff, stats.freshness_days)
        if not fresh.passed:
...
def run(offline: bool = False, only: tuple[str, ...] = (),
    ...
    active_profile = profile.active()
    # Boards that filter by posting age server-side read the window off the
    # active search, so the window this run settled on has to be pinned there
    # too — otherwise they fetch their own and the run pays for adverts it is
    # about to discard.
    if active_profile.freshness_days != days:
        active_profile = replace(active_profile, freshness_days=days)
        profile.set_active(active_profile)
